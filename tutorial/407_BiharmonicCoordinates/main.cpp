...
int main(int argc, char *argv[])
{
  ...
  if (!readMESH("../../data/octopus-low.mesh", low.V, low.T, low.F))
  {
    cout << "failed to load mesh" << endl;
  }
  if (!readMESH("../../data/octopus-high.mesh", high.V, high.T, high.F))
  {
    cout << "failed to load mesh" << endl;
  }
  ...
  // Precomputation
  {
    ...
    for_each(high.F.data(), high.F.data() + high.F.size(), [&I](int &a)
             { a = I(a); });
    for_each(b.data(), b.data() + b.size(), [&I](int &a)
             { a = I(a); });
  ...
  C << RowVector3d(0.8, 0.5, 0.2).replicate(low.F.rows(), 1),
